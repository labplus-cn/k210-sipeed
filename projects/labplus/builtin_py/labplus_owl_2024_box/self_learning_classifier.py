...
class Self_learning_classifier(object):
  ...
  def add_class_img(self):
    ...
    Draw_CJK_String('按S1键按顺序添加分类图片', 45, 5, self.img, color=(0, 255, 0))
    if self.key.value() == 0:
        time.sleep_ms(30)
        if self.key.value() == 0:
          index = self.classifier.add_class_img(self.img)
          # print("add class self.img:", index)
          Draw_CJK_String('添加分类图片，id：{0}'.format(index), 45, 20, self.img, color=(0, 255, 0))
  ...
  def add_sample_img(self):
    ...
    # while True:
    self.img = self.sensor.snapshot()
    Draw_CJK_String('按S2键添加训练集图片', 45, 5, self.img, color=(0, 0, 200))
    if self.key_b.value() == 0:
        time.sleep_ms(30)
        if self.key_b.value() == 0:
          index = self.classifier.add_sample_img(self.img)
          Draw_CJK_String('添加训练集图片{0}'.format(index+1), 45, 20, self.img, color=(0, 0, 200))
          self.lcd.display(self.img)
          time.sleep_ms(2000)
  ...
  def predict(self):
    gc.collect()
    self.img = self.sensor.snapshot()
    Draw_CJK_String('识别中...', 45, 5, self.img, color=(0, 255, 0))
    res_index = None
    min_dist = None
    try:
    ...
    if res_index >= 0 and min_dist <= self.threshold :
      ...
      if(min_dist <= self.threshold):
        Draw_CJK_String('识别到id:{0}'.format(res_index), 45, 20, self.img, color=(0, 255, 0))
        print('识别到id:{0}'.format(res_index))
        self.lcd.display(self.img)
        return res_index,min_dist
    else:
        Draw_CJK_String('未识别到', 45, 20, self.img, color=(0, 255, 0))
        self.lcd.display(self.img)
        return None,None
  ...
  def save_classifier(self, name="classes.classifier"):
    ...
    self.img = self.sensor.snapshot()
    Draw_CJK_String('保存模型成功', 45, 35, self.img, color=(0, 200, 0))
    self.lcd.display(self.img)
    time.sleep_ms(1500)
  ...
  def load_classifier(self, name="classes.classifier"):
    ...
    if file_exit:
      ...
      self.img = self.sensor.snapshot()
      Draw_CJK_String('载入已保存模型', 45, 35, self.img, color=(0, 200, 0))
      self.lcd.display(self.img)
      time.sleep_ms(1500)
    else:
      self.img = self.sensor.snapshot()
      Draw_CJK_String('模型文件不存在', 45, 35, self.img, color=(180, 0, 0))
      self.lcd.display(self.img)
      time.sleep_ms(1500)
