...
class MP3(object):
    ...
    def __init__(self, tx=-1, rx=-1, uart_num=1):
        if(uart_num==1):
            fm.register(tx, fm.fpioa.UART1_TX, force=True)
            fm.register(rx, fm.fpioa.UART1_RX, force=True)
        else:
            fm.register(tx, fm.fpioa.UART2_TX, force=True)
            fm.register(rx, fm.fpioa.UART2_RX, force=True)
       
        self.uart = UART(UART.UART1+uart_num-1, 9600)
        self.volume = 25
...
class IRRecv(object):
    ...
    def __init__(self, rx, uart_id=1):
        ...
        if(uart_id==1):
            ...
            fm.register(rx, fm.fpioa.UART2_RX, force=True)
        self.uart = UART(uart_id-1, baudrate=115200, rx=rx)
...
class IRTrans(object):
    ...
    def __init__(self, tx, uart_id=2):
        if(uart_id==1):
            ...
            fm.register(tx, fm.fpioa.UART2_TX, force=True)
        self.uart = UART(uart_id-1, baudrate=115200, tx=tx)
